...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    // ========================================================
    // 1. TANGKAP ID DOKTER LANGSUNG DARI FORM
    // ========================================================
    $id_dokter_final = (int)$_POST['id_dokter'];

    if (empty($id_dokter_final)) {
        header("Location: " . BASE_URL . "resep/tambah.php?status=gagal&msg=" . urlencode("Nama Dokter wajib dipilih."));
        exit;
    }

    // ========================================================
    // 2. MULAI TRANSAKSI UNTUK RESEP DAN STOK OBAT
    // ========================================================
    ...
    try {
        ...
        // Insert Header Resep (Termasuk id_dokter_final)
        $sql_header = "INSERT INTO tbl_resep_header 
                         (tgl_resep, nama_pasien, id_poli, id_dokter, id_pelayanan, id_user_pencatat, 
                          kelengkapan_resep, kesalahan_resep, sesuai_formularium)
                       VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)";
        $stmt_header = $pdo->prepare($sql_header);
        $stmt_header->execute([
            $tgl_resep, $nama_pasien, $id_poli, $id_dokter_final, $id_pelayanan, $id_user_pencatat,
            $kelengkapan_resep, $kesalahan_resep, $sesuai_formularium
        ]);
        
        $id_resep_baru = $pdo->lastInsertId();

        // Siapkan Statement untuk Detail Obat
        ...
        foreach ($obat_ids as $index => $id_obat) {
            ...
            if (!$stok_saat_ini || $stok_saat_ini['stok_akhir'] < $jumlah_keluar) {
                // --- PERBAIKAN: Ambil Kode & Nama Obat dari Database ---
                ...
                throw new Exception("Stok obat ($nama_lengkap_obat) tidak mencukupi di unit ini. Transaksi dibatalkan.");
            }
        ...
        exit;
            ...
            // --- TAMBAHKAN BARIS INI AGAR DATA TIDAK HILANG ---
